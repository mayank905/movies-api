...
async function insert(){
    ...
    //     // console.log(movies[j][10])
       ...
       // movies[j].splice(10,1,k)

        let values=movies[j].slice(1)
        values.splice(8,1,k+1)
        console.log(values)
        let insertQuery =await pgClient.query(insertMovies,values)
        // // let values=[movies[j][1],movies[j][2],movies[j][3],movies[j][4],movies[j][5],movies[j][6],movies[j][7],movies[j][8],movies[j][9],retrieveQuery.rows,movies[j][11]]
